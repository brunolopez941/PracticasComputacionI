...
int CalcularCargaApH(float& ph, grupos todosGrupos)
{
    int numCargas = todosGrupos.size();
    int neta = 2;
    int aux = 0;
    for (int i = 0; i < numCargas; i++) {
        if (ph < todosGrupos[i].GetCarga()) {
            aux = neta - i;
            neta = aux;
            break;
        }
        else if(ph>todosGrupos[2].GetCarga()){
            neta = -1;
        }
    }
    /*
    if (ph < todosGrupos[0].GetCarga()) {
        neta = 2;
    }
    else if (ph < todosGrupos[1].GetCarga()) {
        neta = 1;
    }
    else if (ph < todosGrupos[2].GetCarga()) {
        neta = 0;
